...
import { Activity, MapPin, MoreVertical, Phone } from 'lucide-react'
...
export default function PatientCard({ patient, index, onClick }: PatientCardProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
      <Card onClick={onClick} className="group relative bg-card rounded-xl border border-border p-8 hover:shadow-2xl hover:shadow-zinc-200/50 transition-all duration-500 cursor-pointer overflow-hidden">
        <div className="flex justify-between items-start mb-6">
          <div className="w-16 h-16 rounded-lg bg-muted border border-border flex items-center justify-center text-primary font-black text-2xl group-hover:bg-primary group-hover:text-background transition-all">{patient.name[0]}</div>
