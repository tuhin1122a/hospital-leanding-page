...
export default function ApprovalCenter({ approvals, onAction, isLoading }: ApprovalCenterProps) {
  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-10 shadow-sm relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
        {isLoading ? [1,2,3].map(i => <div key={i} className="h-48 rounded-xl bg-muted animate-pulse" />) : approvals.length === 0 ? (
          <div className="col-span-full text-center py-20 bg-muted/20 rounded-xl border border-dashed border-border"><UserCheck size={48} className="mx-auto text-muted-foreground/30 mb-4" /><p className="text-lg font-bold text-muted-foreground">{t('No pending approval requests')}</p></div>
        ) : approvals.map((req) => (
          <motion.div key={req.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="p-6 rounded-xl border border-border bg-card/50 hover:shadow-xl transition-all">
