...
export default function ThemePicker({ current, onSelect }: ThemePickerProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card rounded-[2.5rem] border border-border p-8 shadow-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
        {themes.map((th) => (
          <button key={th.id} onClick={() => onSelect(th.id)} className={`relative flex flex-col items-center gap-3 p-5 rounded-[1.5rem] border-2 transition-all duration-300 group ${current === th.id ? 'border-primary bg-primary/5 shadow-lg shadow-primary/10' : 'border-border bg-muted/50 hover:border-primary/40 hover:bg-muted'}`}>
