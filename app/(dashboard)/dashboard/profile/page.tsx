...
export default function ProfilePage() {
  ...
  const { data: user } = useQuery({ queryKey: ['me'], queryFn: () => fetch('http://localhost:5000/auth/me', { headers: authHeader() }).then(r => r.json()) })

  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      const fd = new FormData(); fd.append('file', file)
      return fetch('http://localhost:5000/users/profile-pic', { method: 'POST', headers: authHeader(), body: fd }).then(r => r.json())
    },
