      ...
      <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
        <div className="p-8 border-b border-border/50 flex flex-col lg:flex-row items-center gap-6">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <Button variant="outline" className="h-14 px-6 rounded-2xl border-border font-bold text-muted-foreground"><Filter size={18} className="mr-2" /> {t('Filter')}</Button>
            {role !== 'PATIENT' && <Button onClick={() => setShowUpload(true)} className="h-14 px-8 rounded-2xl bg-primary text-background font-black">{t('Upload File')}</Button>}
          </div>
        </div>
        {isLoading ? <div className="p-20 text-center animate-pulse font-black text-muted-foreground/50">{t('SYNCING CLINICAL DATA...')}</div> : <RecordsTable records={filtered} role={role} />}
      </div>
