...
export default function PatientsPage() {
  ...
  const { data: patients = [], isLoading } = useQuery({ queryKey: ['patients'], queryFn: () => fetch(API_BASE, { headers: authHeader() }).then(r => r.json()).then(d => Array.isArray(d) ? d : Array.isArray(d?.data) ? d.data : Array.isArray(d?.patients) ? d.patients : []) })
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-6">
        <div className="relative flex-grow w-full"><Search size={20} className="absolute left-6 top-1/2 -translate-y-1/2 text-muted-foreground/70" /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t("Search by name, ID, or phone number...")} className="w-full h-16 pl-16 rounded-2xl bg-card border border-border outline-none font-bold shadow-sm" /></div>
        <div className="flex items-center gap-3 w-full lg:w-auto"><Button variant="outline" className="h-16 px-8 rounded-2xl font-black text-muted-foreground"><Filter size={18} className="mr-2 text-primary" /> {t('Filter')}</Button></div>
      </div>
      {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">{[1,2,3].map(i => <div key={i} className="h-80 rounded-2xl bg-muted animate-pulse" />)}</div> : filtered.length === 0 ? <div className="text-center py-20 bg-muted/30 rounded-2xl border border-dashed"><User size={48} className="mx-auto text-muted-foreground/40 mb-4" /><p className="text-xl font-bold">{t('No patients found')}</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">{filtered.map((p:any, i:number) => <PatientCard key={p.id} patient={p} index={i} onClick={() => { setSelected(p); setShowHistory(true) }} />)}</div>}
