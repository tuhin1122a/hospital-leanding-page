...
export default function SettingsPage() {
  ...
  const updateProfileMutation = useMutation({
    mutationFn: (name: string) => fetch(`${process.env.NEXT_PUBLIC_API_URL}/users/update-profile`, { 
      method: 'POST', 
      headers: authHeader(), 
    ...
    <div className="max-w-6xl mx-auto space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <Button 
          ...
          className="h-14 px-10 rounded-2xl bg-primary text-primary-foreground font-black shadow-xl shadow-primary/20"
        >
          {updateProfileMutation.isPending ? t('Saving...') : <><Sparkles size={18} className="mr-2" /> {t('Save Changes')}</>}
        </Button>
      </div>
      <SettingsHero user={userProfile} isUploading={updatePicMutation.isPending} onFileSelect={(f) => updatePicMutation.mutate(f)} />
      <GeneralPreferences user={userProfile} lang={lang} setLang={setLang} onNameChange={(v) => setNewName(v)} />
