    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground font-medium text-lg mt-1">{t('Manage hospital revenue, insurance claims, and patient invoices')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         {[
           { label: t('Total Earnings'), value: '$1.2M', icon: DollarSign, color: 'text-emerald-500', trend: '+14% this month' },
           { label: t('Operational Cost'), value: '$450K', icon: Wallet, color: 'text-amber-500', trend: 'Maintenance & Utility' },
           { label: t('Staff Salaries'), value: '$320K', icon: FileCheck, color: 'text-blue-500', trend: '142 active personnel' },
           { label: t('Net Profit'), value: '$430K', icon: CreditCard, color: 'text-primary', trend: '+8% from last month' },
         ].map((stat, i) => (
           <Card key={i} className="p-8 border-none rounded-[2.5rem] bg-card shadow-xl shadow-zinc-200/40 relative overflow-hidden group">
              ...
              <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-muted rounded-full group-hover:scale-125 transition-transform duration-700 -z-0" />
           </Card>
         ))}
      </div>
      ...
      <div className="bg-card rounded-[2.5rem] border border-border shadow-sm overflow-hidden">
        <div className="p-10 flex flex-col lg:flex-row items-center justify-between gap-8 border-b border-border/50">
           ...
           <div className="flex items-center gap-4 w-full lg:w-auto">
              <Button variant="outline" className="h-14 px-8 rounded-2xl border-border font-bold text-muted-foreground">
           ...
           <Table>
              <TableHeader>
                 <TableRow className="hover:bg-transparent border-border/50">
                    ...
                    <TableHead className="px-10 py-5 font-black uppercase tracking-widest text-[10px] text-muted-foreground/70">{t('Amount')}</TableHead>
                    <TableHead className="px-10 py-5 font-black uppercase tracking-widest text-[10px] text-muted-foreground/70">{t('Payment')}</TableHead>
                 ...
                 {invoices.map((inv, i) => (
                   <TableRow key={i} className="group border-border/50 hover:bg-muted/50 transition-colors">
                      <TableCell className="px-10 py-6">
                         <span className="font-black text-card-foreground tracking-tight">{inv.id}</span>
                      </TableCell>
                      <TableCell className="px-10 py-6 font-bold text-muted-foreground">{inv.patient}</TableCell>
                      <TableCell className="px-10 py-6 text-sm font-bold text-muted-foreground">{inv.date}</TableCell>
                      <TableCell className="px-10 py-6">
                         <span className="font-black text-card-foreground">{inv.amount}</span>
                      </TableCell>
                      <TableCell className="px-10 py-6 font-bold text-muted-foreground/70 text-xs uppercase tracking-widest">{inv.method}</TableCell>
                      <TableCell className="px-10 py-6">
                         <Badge className={cn(
                           ...
                           inv.status === 'Pending' ? 'bg-amber-500 text-background' : 'bg-red-500 text-background'
                         )}>
                           {inv.status}
                         </Badge>
                      </TableCell>
