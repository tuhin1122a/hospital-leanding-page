    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <Button className="h-14 px-8 rounded-2xl bg-primary hover:bg-primary/90 text-background font-black shadow-xl shadow-primary/20 transition-all">
          <Plus size={20} className="mr-2" /> {t('Add Staff Member')}
        </Button>
      </div>
              ...
              <div className="p-8">
                <div className="flex justify-between items-start mb-6">
                  <div className="relative">
                    <img src={member.image} alt={member.name} className="w-16 h-16 rounded-2xl object-cover border-2 border-card shadow-lg" />
                    <div className={cn(
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <Badge variant="outline" className="bg-muted border-border text-muted-foreground font-black text-[10px] uppercase tracking-widest px-3 py-1 rounded-full">
                      {member.dept} {t('Unit')}
                    </Badge>
                    <span className="text-[10px] font-black text-muted-foreground/70 uppercase tracking-widest">{member.id}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-muted/50 rounded-2xl p-4">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70 mb-1">{t('Tenure')}</p>
                    <p className="text-md font-black text-card-foreground">{member.experience}</p>
                  </div>
                  <div className="bg-muted/50 rounded-2xl p-4">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/70 mb-1">{t('Status')}</p>
                    <p className={cn(
                      "text-md font-black",
                      member.status === 'On Shift' ? 'text-emerald-600' : 'text-muted-foreground'
                    )}>{t(member.status)}</p>
                  </div>
                </div>
              </div>
